...
return params;},clear:function(additionalParams){if(DW.clearCalled){return;}
var params=this.mergeParams(DW.pageParams,additionalParams);d=new Date();params.ts=d.getTime();if(!params.sid){params.sid=params.siteid;delete params.siteid;}
if(!params.tag){}
...
var src=DW.buildUrl(DW.clearPath,DW.toQueryString(params));var img=new Image(1,1);img.src=src;DW.clearCalled=true;return src;},buildUrl:function(path,queryString){return DW.protocol+"//"+DW.host+path+"?"+queryString;},toQueryString:function(params){var qs="";var first=true;for(var p in params){val=params[p];if(val){if(!first)qs+="&";qs+=p+"="+encodeURIComponent(val);first=false;}}
return qs;}};DW.clearCalled=false;DW.protocol="http:";if(window.location.protocol=="https:"){DW.protocol="https:";}
